...
1. Load the email message using the `MapiMessage` فصل:
...
يستبدل `"path/to/your/email.msg"` with the actual path to your email message file.
...
To check if an email is a TNEF message, use the `IsTnefMessage()` method of the `MapiMessage` فصل:
