...
### Aspose Email Java チュートリアル: Maven 経由のインストール
...
- **無料トライアル** – 機能を試すためにトライアルから開始。  
- **一時ライセンス** – 長期テスト用に一時ライセンスを取得。  
- **購入** – 本番環境での使用には正式ライセンスの購入を検討。
...
### PreserveTnefAttachments 機能を使用した EML の読み込み
...
- **添付ファイルの欠落** – ファイルパス（`dataDir`）と読み取り権限を再確認してください。  
- **予期しない変換** – メッセージを読み込む前に `setPreserveTnefAttachments(true)` が呼び出されているか確認してください。  
...
2. **法務・コンプライアンスソフトウェア** – 文書保持ポリシーに従い、ネイティブな TNEF ファイルを保存。  
3. **カスタマーサポートツール** – クライアントメールを添付ファイルを変更せずに転送。

## パフォーマンスに関する考慮事項

大量のメールを Aspose.Email で処理する際のポイント：

- **オブジェクトの破棄** – 各メッセージ処理後に `eml.dispose()`（またはガベージコレクタに任せる）を実行。  
- **メモリ設定** – 処理対象メール量に応じて JVM のヒープサイズ（`-Xmx`）を調整。
